    ...
    var vm = new Vue({
        ...
        mounted() {
            ...
            document.onkeydown = function (event) {
                ...
                if (shiftKey && altKey && keyCode == 81) {
                    //添加
                    //console.log("按下了alt shift + Q键");
                ...
                } else if (shiftKey && altKey && keyCode == 65) {
                    // alt shift A 快速添加（不出现面板）
                    const msg = window.getSelection().toString();
                    origan_data = GM_getValue('priate_script_note_data')
                    _this.note.content = msg
                    _this.note.id = _this.setting.number++
                    _this.note.time = getNowFormatDate()
                    _this.note.host = location.host
                    ...
                    GM_setValue('priate_script_note_setting', _this.setting)
                    _this.clearNote()
                } else if (shiftKey && altKey && keyCode == 83) {
                    console.log("alt shift S")
                    // alt shift S 快速缓存到本地
                    var elementsByTagName = document.getElementsByTagName("html");
                    GM_setClipboard(elementsByTagName[0].outerHTML)

                    origan_data = GM_getValue('priate_script_note_data')
                    _this.note.id = _this.setting.number++
                    _this.note.content = "空"
                    _this.note.time = getNowFormatDate()
                    _this.note.host = location.host
                    origan_data.push(_this.note)
                    GM_setValue('priate_script_note_data', origan_data)
